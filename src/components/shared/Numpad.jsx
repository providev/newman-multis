    ...
    <div className="grid grid-cols-3 gap-1.5 sm:gap-2 h-full content-center">
      {nums.map(n => (
        <button key={n} onClick={() => onInput(String(n))}
          className="bg-slate-100 text-indigo-900 text-fluid-2xl sm:text-3xl font-bold py-1.5 sm:py-4 rounded-xl active:bg-slate-300">
          {n}
        </button>
      ))}
      ...
      <button onClick={() => onInput('0')}
        className="bg-slate-100 text-indigo-900 text-fluid-2xl sm:text-3xl font-bold py-1.5 sm:py-4 rounded-xl active:bg-slate-300">0</button>
      <button onClick={() => onInput('ok')}
        className="bg-green-500 text-white text-sm sm:text-xl font-bold py-1.5 sm:py-4 rounded-xl active:bg-green-700 shadow-sm">OK</button>
    </div>
  )
}
