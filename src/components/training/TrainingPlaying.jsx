...
export default function TrainingPlaying({ state, config, handleAnswer, onFinish, onBack }) {
  const { phase, questions, currentIndex, score, totalTime, timeLeft, feedbackType, feedbackAnswer } = state
  ...
  useEffect(() => {
    if (phase === 'finished' && !finishedCalled.current) {
      ...
      const t = config.timeMode === 'stopwatch' ? totalTime : config.countdownSecs
      onFinish(score, t)
    }
    if (phase !== 'finished') {
  ...
  const timeDisplay = config.timeMode === 'countdown' ? `${timeLeft}sg` : `${totalTime}s`
  ...
  return (
    <div className="h-full grid grid-rows-[auto_1fr_auto]">
      ...
      <div className="px-3 py-2 sm:p-4 bg-white rounded-t-2xl sm:rounded-t-3xl shadow-[0_-10px_40px_-15px_rgba(0,0,0,0.1)] shrink-0">
        {config.gameMode === 'test' && q ? (
          <OptionButtons options={q.options} onSelect={onOptionSelect} />
        ) : (
